...
#define kStandardLineWidth 3.0f
#define kStandardCornerSize 10.0f
...
@interface NHGuideLayer ()

@property(nonatomic, weak, readwrite) id<NHGuideLayerDelegate> guideLayerDelegate;
...
- (id)initWithDelegate:(id<NHGuideLayerDelegate>)guideLayerDelegate {
    if((self = [super init])) {
        ...
        NSArray *cornerLayers = [NSArray arrayWithObjects:
                               _topLayer,
                               _bottomLayer,
                               _leftLayer,
                               _rightLayer,
                               nil];
        for(CAShapeLayer *layer in cornerLayers) {
        ...
        for(CAShapeLayer *layer in edgeLayers) {
            layer.frame = CGRectZeroWithSize(self.bounds.size);
            layer.lineCap = kCALineCapButt;
            layer.lineWidth = [self lineWidth]*2;
            layer.fillColor = [UIColor clearColor].CGColor;
            layer.strokeColor = kDefaultGuideColor.CGColor;
...
+ (CGRect)guideFrameForDeviceOrientation:(UIDeviceOrientation)deviceOrientation inViewWithSize:(CGSize)size {
    ...
    if (UIInterfaceOrientationIsPortrait(interfaceOrientation)) {
        cvm_rect guideFrame = cvm_guide_frame(frameOrientation, (float)size.width, (float)size.height);
        return CGRectWithCVMRect(guideFrame);
    }
    else {
        cvm_rect guideFrame = cvm_guide_frame(frameOrientation, (float)size.height, (float)size.width);
        return CGRectWithRotatedCVMRect(guideFrame);
    }
}
...
- (void)setVideoFrame:(NHVideoFrame *)newFrame {
